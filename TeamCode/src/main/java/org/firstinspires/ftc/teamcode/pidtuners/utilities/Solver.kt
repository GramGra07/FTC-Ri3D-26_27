...
class Solver {
    companion object{
        ...
        fun find_pidf_params_pivot_coupled(i: Double, kG:Double, stallTorque: Double, integrandFactor: Double = 100.0, armMass:Double, manipMass:Double, ticksPerMeter: Double, length:Double, coupling: Coupling): PivotPIDFCoefs{
            ...
            val kP = RangePair<Double>(
                i * omega_n * omega_n / stallTorque,
                I_max * omega_n * omega_n / stallTorque
            )
            val kI = RangePair<Double>(
                (kP.low / integrandFactor) / stallTorque,
                (kP.high / integrandFactor) / stallTorque
            )
            val kD = RangePair<Double>(
                2 * zeta * omega_n * i / stallTorque,
                2 * zeta * omega_n * I_max / stallTorque
            )
            val kG =
                RangePair<Double>(kG, armMass * (length / 2) + manipMass * L_max * PhysicsMath.G)
